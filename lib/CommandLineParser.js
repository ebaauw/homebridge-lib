...
class CommandLineParser {
  ...
  help (short, long, helpText) {
    helpText = homebridgeLib.OptionParser.toString(helpText, true)
...
(${this._packageJson.name} v${this._packageJson.version}, node ${process.version})`
  ...
  flag (short, long, callback) {
    ...
    callback = homebridgeLib.OptionParser.toFunction(callback)
  ...
  parameter (name, callback) {
    name = homebridgeLib.OptionParser.toString(name, true)
    callback = homebridgeLib.OptionParser.toFunction(callback)
  ...
  parse (wordList = process.argv.slice(2)) {
    wordList = homebridgeLib.OptionParser.toArray(wordList)
