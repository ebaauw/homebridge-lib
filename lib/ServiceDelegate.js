...
class History extends ServiceDelegate {
  ...
  _heartbeat (beat) {
    if (beat % 600 === this._historyBeat) {
      this._addEntry()
      if (beat % 86400 === this._historyBeat) {
        this._save()
      }
    }
  }
  ...
  _save () {
    ...
    this.debug('saved %d history entries', this._usedMemory)
    // const data = {
    //   firstEntry: this._firstEntry,
    //   lastEntry: this._lastEntry,
    //   usedMemory: this._usedMemory,
  ...
  _load () {
    if (this._context.history != null) {
      this._firstEntry = this._context.history.firstEntry
      this._lastEntry = this._context.history.lastEntry
      this._usedMemory = this._context.history.usedMemory
      this._refTime = this._context.history.refTime
      this._initialTime = this._context.history.initialTime
      this._history = this._context.history.history
      this.debug('restored %d history entries', this._usedMemory)
    }
    // this._loading = true
