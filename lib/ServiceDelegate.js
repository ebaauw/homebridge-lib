...
/** Class for a _Battery_ service delegate.
  ...
  * ------------------ | -------------------------------------- | ----------
  * `name`             | `Characteristics.hap.Name`             |
  * `batteryLevel`     | `Characteristics.hap.BatteryService`   |
  * `chargingState`    | `Characteristics.hap.ChargingState`    |
...
class Battery extends ServiceDelegate {
  ...
  constructor (accessoryDelegate, params = {}) {
    ...
    this.addCharacteristicDelegate({
      ...
      value: this.Characteristics.hap.StatusLowBattery.BATTERY_LEVEL_NORMAL
    })
  }
}
...
class Consumption extends ServiceDelegate.History {
  ...
  _addEntry () {
    ...
    if (this.values.consumption != null) {
      ...
      const power = Math.round(delta * 6000) // W
