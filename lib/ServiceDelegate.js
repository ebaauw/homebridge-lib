...
class ServiceDelegate extends homebridgeLib.Delegate {
  ...
  destroy () {
    ...
    for (const key in this._characteristicDelegates) {
      this.removeCharacteristic(key)
    }
    this._accessory.removeService(this._service)
    delete this._accessory.context[this._key]
  }

  /** Add a newly created HomeKit characteristic delegate.
    *
  ...
  /** Remove an associated HomeKit characteristic delegate.
    *
    * @params {!string} key - The characteristic key.
    */
  removeCharacteristic (key) {
    delete this.values[key]
    if (this._characteristicDelegates[key] != null) {
