    ...
    for (let i = 0; i < 11; i++) {
      ...
      if (
        ...
        this.debug(
          ...
          address, this._h.refTime, moment.unix(this._h.refTime + epoch)
        )
...
/** Class for a Raspberry Pi _History_ service delegate.
  ...
  *
  * This delegate creates the history from the associated
  * `Characteristics.eve.CurrentTemperature` and
  * `Characteristics.hap.On` characteristics.
  * @extends ServiceDelegate.History
  * @memberof ServiceDelegate.History
  */
class Rpi extends ServiceDelegate.History {
  ...
  constructor (
    accessoryDelegate,
    params = {},
    temperatureDelegate, onDelegate
  ) {
    super(accessoryDelegate, params)
    if (!(temperatureDelegate instanceof homebridgeLib.CharacteristicDelegate)) {
      throw new TypeError('temperatureDelegate: not a CharacteristicDelegate')
  ...
  get _fingerPrint () { return '02 0102 0e01' }
