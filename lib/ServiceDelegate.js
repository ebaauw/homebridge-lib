...
class ServiceDelegate extends homebridgeLib.Delegate {
  ...
  constructor (accessoryDelegate, params = {}) {
    ...
    const id = subtype == null ? Service.UUID : [Service.UUID, subtype].join('.')

    // Get or create associated Service.
    this._service = subtype == null
    ...
    if (
      subtype == null &&
      accessoryDelegate._accessory.context[id + '.'] != null
    ) {
      accessoryDelegate._accessory.context[id] =
        accessoryDelegate._accessory.context[id + '.']
      delete accessoryDelegate._accessory.context[id + '.']
    }
  ...
  /** Add HomeKit characteristic and associated charactertistic delegate.
    *
    * @param {!object} params - Properties of the HomeKit characteristic.
    * @param {!string} params.key - The key for the characteristic.<br>
    * The characteristic value van be read or written through
    * `this.values[key]`.  The charactertistic delegate can be retrieved
    * through `this.characteristicDelegate(key)`.
    * @param {?*} params.value - The initial value the characteristic.
    ...
    * @param {?function} params.didSet - Function to invoke when the
    * characteristic's value was set.
    */
  addCharacteristic (params) {
    ...
    if (params.didSet != null) {
      characteristicDelegate.on('didSet', params.didSet.bind(this))
    }
  }
  ...
  /** Returns the delegate of the corrresponding HomeKit accessory delegate.
    * returns {AccessoryDelegate}
    */
...
class History extends ServiceDelegate {
  ...
  constructor (accessoryDelegate, params = {}) {
    ...
    this._h = this.context._history
    if (this._h == null) {
      this.context._history = {
        firstEntry: 0,
        lastEntry: 0,
        history: ['noValue'],
        usedMemory: 0,
    ...
    this.addCharacteristic({
      key: 'historyRequest',
      Characteristic: this.Characteristic.eve.HistoryRequest,
      value: params.historyRequest,
      didSet: (value) => { this._onSetHistoryRequest(value) }
    })
    this.addCharacteristic({
      key: 'setTime',
      Characteristic: this.Characteristic.eve.SetTime,
      value: params.setTime
    })
    this.addCharacteristic({
      key: 'historyStatus',
      Characteristic: this.Characteristic.eve.HistoryStatus,
      value: params.historyStatus
    })
    this.addCharacteristic({
      key: 'historyEntries',
      Characteristic: this.Characteristic.eve.HistoryEntries,
  ...
  _onGetEntries (callback) {
    ...
    return callback(null, hexToBase64(dataStream))
  }
...
class Contact extends ServiceDelegate.History {
  ...
  constructor (
    ...
    this.addCharacteristic({
      key: 'resetTotal',
      Characteristic: this.Characteristic.eve.ResetTotal,
      value: params.resetTotal,
      didSet: (value) => { timesOpenedDelegate.value = 0 }
    })
  }
...
/** Class for an Eve Thermo _History_ service delegate.
  ...
  *
  * This delegate creates the history from the associated
  * `Characteristic.hap.CurrentTemperature`,
  * `Characteristic.hap.TargetTemperature`, and
  * `Characteristic.eve.ValvePosition` characteristics.
  * @extends ServiceDelegate.History
  * @memberof ServiceDelegate.History
  */
class Thermo extends ServiceDelegate.History {
  /** Create a new instance of an Eve Thermo _History_ service delegate.
    ...
    * delegate of the associated `Characteristic.hap.CurrentTemperature`
    * characteristic.
    * @param {!CharacteristicDelegate} targetTemperatureDelegate - A reference
    * to the delegate of the associated `Characteristic.hap.TargetTemperature`
    * characteristic.
    * @param {!CharacteristicDelegate} valvePositionDelegate - A reference to
    * the delegate of the associated `Characteristic.eve.ValvePosition`
    * characteristic.
    */
  constructor (
    accessoryDelegate, params = {},
