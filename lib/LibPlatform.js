...
const context = {
  ...
  pluginName: '',
  pluginVersion: '',
  libName: homebridgeLib.packageJson.name,
  libVersion: homebridgeLib.packageJson.version,
  nodeVersion: process.version,
  recommendedNodeVersion: homebridgeLib.packageJson.engines.node,
  homebridgeVersion: '',
  recommendedHomebridgeVersion: homebridgeLib.packageJson.engines.homebridge,
  maxAccessories: 99,
  maxServices: 99,
...
/**
  ...
  *
  * `LibPlatform` provides the following features to a platform plugin:
  ...
  * @extends LibObject
  */
class LibPlatform extends homebridgeLib.LibObject {
  /**
   ...
   * @param {LibPlatform} Platform - The constructor of the platform plugin.
   */
   ...
   * for the plugin.
   * @param {object} consigJson - The contents of the platform object from
  ...
  _main () {
    ...
    this._heartbeat = setInterval(() => {
      ...
      for (const id in this._accessoryDelegates) {
        /**
          * Emitted evert seconds.
          * @event libAccessory#heartbeat
          * @param {number} beat - The sequence number of this heartbeat.
          */
  ...
  configureAccessory (accessory) {
    ...
    if (this._configJson != null) {
       ...
       * delegate.
       * @event LibPlatform#accessoryRestored
       * @param {!string} className - The {@link LibAccessory#className class}
       * of the accessory delegate.
       * @param {object} context - The accessory
       * {@link LibAccessory#context context}.
       */
  ...
  _upnpConfig (config) {
    ...
    this._upnpMonitor = new homebridgeLib.UpnpMonitor(options)
    ...
    this._upnpMonitor.on('deviceAlive', (address, obj, message) => {
       ...
       * @event LibPlatform#upnpDeviceAlive
    ...
    this._upnpMonitor.on('deviceFound', (address, obj, message) => {
       ...
       * @event LibPlatform#upnpDeviceFound
       * @param {string} address - The device's IP address.
