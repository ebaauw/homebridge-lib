...
module.exports = class LibPlatform extends LibObject {
  ...
  onExit (status) {
    this.log('homebridge exiting with status %d', status)
  }
  ...
  async _identify () {
    ...
    const nodeVersion = LibPlatform.minVersion(context.recommendedNodeVersion)
    const homebridgeVersion = LibPlatform.minVersion(
    ...
    if (context.homebridgeVersion !== homebridgeVersion) {
      this.warn(
        'not using recommended homebridge version v%s', homebridgeVersion
      )
    }
    const n = Object.keys(this._accessories).length
    if (n > 0) {
  ...
  async _checkLatest () {
    const npmRegistry = new homebridgeHueUtils.RestClient({
  ...
  _cleanup () {
    if (Object.keys(this._accessories).length > 0) {
      this.log('cleanup cached accessories')
      let accessoryList = []
  ...
  _upnpConfig (config) {
    ...
    this._upnpMonitor = new homebridgeHueUtils.UpnpMonitor(this, this._upnp)
  ...
  _message (level, namePrefix, format, ...args) {
    ...
    switch (level) {
      ...
      case 'warning':
        ...
        break
      case 'error':
        message = namePrefix + 'error: ' + message
