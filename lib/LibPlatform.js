...
const context = {
  ...
  pluginName: packageJson.name,       // Name of the plugin, 'homebridge-xxx'
  pluginVersion: packageJson.version, // Version of the plugin.
...
module.exports = class LibPlatform extends LibObject {
  ...
  _identify() {
    ...
    if (!semver.satisfies(
      context.nodeVersion, context.recommendedNodeVersion
    )) {
      this.warning(
        'not using recommended node LTS version %s',
        context.recommendedNodeVersion
      );
    }
    if (!semver.satisfies(
      context.homebridgeVersion, context.recommendedHomebridgeVersion
    )) {
      this.warning(
        'not using recommended homebridge version %s',
