...
module.exports = class LibPlatform extends homebridgeLib.LibObject {
  ...
  constructor (log, configJson, homebridge) {
    ...
    this._eve = new homebridgeLib.EveHomeKitTypes(homebridge)
    this._my = new homebridgeLib.MyHomeKitTypes(homebridge)
  ...
  configureAccessory (accessory) {
    this.debug('cached accessory: %j', accessory.context)
  ...
  _unregisterAccessory (accessory) {
    this.emit(
      'cleanup', accessory.context.className,
      accessory.context.id, accessory.context.context
    )
  ...
  _cleanup () {
    ...
    if (n > 0) {
      ...
      for (const key in this._accessories) {
        this.debug(
          'cleaning up %s %s %j', this._accessories[key].context.className,
          this._accessories[key].context.id,
          this._accessories[key].context.context
        )
        this.emit(
          'cleanup', this._accessories[key].context.className,
          this._accessories[key].context.id,
          this._accessories[key].context.context
        )
  ...
  _upnpConfig (config) {
    ...
    this._upnpMonitor.on('deviceAlive', (address, obj, message) => {
      this.emit('upnpDeviceAlive', address, obj, message)
    })
    this._upnpMonitor.on('deviceFound', (address, obj, message) => {
      this.emit('upnpDeviceFound', address, obj, message)
    })
  }
