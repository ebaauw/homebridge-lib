...
class CharacteristicDelegate extends homebridgeLib.Delegate {
  ...
  get _notifyOnly () {
    return this._characteristic == null
      ? false
      : this._characteristic.eventOnlyCharacteristic
    // This doesn't work because ProgrammableSwitchEvent has READ.
  ...
  /** Current log level (of the associated accessory delegate).
    ...
    */
  get logLevel () {
    return this._serviceDelegate._accessoryDelegate._logLevel
  }
  ...
  set value (value) {
    ...
    if (this._notifyOnly) {
      this._log(
        ...
        ['Single Press', 'Double Press', 'Long Press'][value]
      )
