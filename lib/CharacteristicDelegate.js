...
/** Class for HomeKit characteristic delegates.
  *
...
class CharacteristicDelegate extends homebridgeLib.Delegate {
  ...
  set value (value) {
    ...
    if (!this._silent) {
      if (this._notifyOnly || this.value == null) {
        this.log(
          'set %s to %s%s', this.displayName,
          value, this._unit
        )
      } else {
        this.log(
          'set %s from %s%s to %s%s', this.displayName,
  ...
  async _onGet (callback) {
    ...
    const timeout = setTimeout(() => {
      timedOut = true
      this.warn(
        'get %s: timed out - return previous value %s%s',
    ...
    if (timedOut) {
      this.log(
        'get %s: ignore %s%s - timed out', this.displayName, value, this._unit
      )
    } else {
      // Return value to HomeKit.
      this.log('get %s return %s%s', this.displayName, value, this._unit)
      callback(null, value)
    }
  ...
  async _onSet (value, callback) {
    ...
    if (!this._silent) {
      if (this._writeOnly || this.value == null) {
        this.log('%s changed to %s%s', this.displayName, value, this._unit)
      } else {
        this.log(
          '%s changed from %s%s to %s%s', this.displayName,
          this.value, this._unit, value, this._unit
        )
      }
    }
