...
class CharacteristicDelegate extends homebridgeLib.Delegate {
  ...
  constructor (serviceDelegate, params = {}) {
    ...
    this._service.addOptionalCharacteristic(Characteristic)
    ...
    if (this._characteristic.listenerCount('set') > 1) {
      this.warn('%s: %d listeners', this._characteristic.displayName, this._characteristic.listenerCount('set'))
    }
  ...
  get displayName () { return this._characteristic.displayName }
