  ...
  static get History () {
    return class History extends LibService {
      ...
      _addEntry () {
        ...
        if (this._refTime === 0) {
          this._refTime = this._entry.time - EPOCH_OFFSET
        ...
        const value = util.format(
          ...
          numToHex(swap32(this._entry.time - this._refTime - EPOCH_OFFSET), 8),
        ...
        for (var i = 0; i < 11; i++) {
          ...
          if (
            ...
            this.debug(
              ...
              moment.unix(this._refTime + EPOCH_OFFSET)
            )
            ...
            this.debug(
              ...
              address, this._history[address].time - this._refTime - EPOCH_OFFSET,
      ...
      static get Energy () {
        return class Energy extends LibService.History {
          ...
          set power (value) { this._entry.power = value }
        }
      }
      ...
      static get Weather () {
        return class Weather extends LibService.History {
          constructor (libAccessory, params = {}) {
            params.type = 'weather'
            super(libAccessory, params)
            this._entry = { time: 0, temp: 0, humidity: 0, pressure: 0 }
          }
          ...
          _entryStream (entry) {
            return util.format(
              ' 10 %s%s07%s%s%s',
              numToHex(swap32(this._currentEntry), 8),
              numToHex(swap32(entry.time - this._refTime - EPOCH_OFFSET), 8),
