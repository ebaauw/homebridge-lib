...
class History extends ServiceDelegate {
  ...
  async _onSetHistoryRequest (value) {
    ...
    this._currentEntry = Math.max(1, entry)
    this._transfer = true
  }
  ...
  async _onGetEntries () {
    ...
    for (let i = 0; i < 11; i++) {
      const index = this._h.memorySize === 0
        ? 1
        : this._currentEntry % this._h.memorySize
      if (this._currentEntry === this._h.firstEntry) {
        ...
        this.debug(
          'History Entries (%d/11): entry %d (index: %d): %j (%s)',
          i, this._currentEntry, index, entry,
          new Date(1000 * entry.time).toString().slice(0, 24)
        )
        const b = this._entryToBuffer(entry)
