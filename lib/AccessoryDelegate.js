...
class AccessoryDelegate extends homebridgeLib.Delegate {
  ...
  constructor (platform, params = {}) {
    ...
    this.once('initialised', () => {
      this._platform.log('%s: logLevel %d', this.name, this._logLevel)
      const services = []
  ...
  _identify () {
    if (this._identifyTimer != null) {
      ...
      const oldLoglevel = this._logLevel
      this._context.logLevel += 1
      this._context.logLevel %= 4
      this._platform.log(
        '%s: set loglevel from %d to %d', this.name, oldLoglevel, this._logLevel
      )
