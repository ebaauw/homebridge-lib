...
module.exports = class LibCharacteristic extends LibObject {
  ...
  constructor(parent, params) {
    super(parent._platform);
    this._parent = parent;              // Associated LibService.
    this._context = parent._context;    // Persisted storage.
    this._key = params.key;             // Name of associated property.
    this._unit = params.unit || '';     // Unit of associated Characteristic.
    ...
    this._characteristic = parent._service.getCharacteristic(Char) ||
      parent._service.addCharacteristic(Char);
  ...
  _isValid(value) {
    ...
    for (var i = 0; i < props.validValues.length; i++) {
      const validValue = props.validValues[i];
      if (value === validValue) {
        return true;
      }
    }
