...
module.exports = class LibCharacteristic extends homebridgeLib.LibObject {
  ...
  constructor (libService, params = {}) {
    ...
    this._values = libService._context
    this._key = params.key
    this._unit = params.unit == null ? '' : params.unit

    // Get or create associated Characteristic.
    const Char = params.Characteristic
    this._characteristic = libService._service.getCharacteristic(Char)
    if (this._isWriteable) {
      this._characteristic.on('set', (value, callback) => {
        ...
        libService.emit('didSet', this._key, value)
        return callback()
      })
    }
