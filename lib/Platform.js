...
class Platform extends homebridgeLib.Delegate {
  ...
  static loadPlatform (homebridge, packageJson, platformName, Platform) {
    if (context.homebridge == null) {
      ...
      homebridge.on('didFinishLaunching', Platform._start)
      homebridge.on('shutdown', Platform._stop)
    }
  ...
  static _start () {
    context.heartbeatStart = new Date()
    setTimeout(() => { Platform._beat(-1) }, 1000)
  }
  ...
  static _stop () {
    ...
    for (const plugin in context.plugins) {
      context.plugins[plugin].platform.emit('shutdown')
    }
  }
  ...
  static _beat (beat) {
    beat += 1
    const drift = new Date() - context.heartbeatStart - 1000 * (beat + 1)
    for (const plugin in context.plugins) {
      /** Emitted every second.
        ...
        */
      context.plugins[plugin].platform.emit('heartbeat', beat, drift)
    }
    if (context.shuttingDown) {
      return
    }
    ...
    if (beat % context.saveInterval === 30) {
      ...
      context.homebridge.updatePlatformAccessories()
    }
  }
