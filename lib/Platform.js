...
const context = {
  ...
  recommendedNodeVersion:
    semver.minVersion(packageJson.engines.node).toString(),
  recommendedHomebridgeVersion:
    semver.minVersion(packageJson.engines.homebridge).toString(),
  saveInterval: 3600,
...
class Platform extends homebridgeLib.Delegate {
  ...
  constructor (log, configJson, homebridge) {
    ...
    const myContext = context.plugins[this.className]
    if (myContext.packageJson.name === context.libName) {
      ...
      this._recommendedLibVersion = semver.minVersion(
        myContext.packageJson.peerDependencies[context.libName]
      ).toString()
    }
    this._accessories = {}
    this._accessoryDelegates = {}

    if (configJson == null && !this._isHomebridgeLib) {
      return
    }
    ...
    if (context.plugins.Platform == null) {
  ...
  get Service () { return context.Service }
  ...
  _beat (beat) {
    ...
    for (const plugin in context.plugins) {
      const platform = context.plugins[plugin].platform
      if (platform != null) {
  ...
  _identify () {
    if (this._isHomebridgeLib) {
      ...
      if (context.libVersion !== this._recommendedLibVersion) {
        this.warn(
          'not using recommended %s v%s', context.libName,
          this._recommendedLibVersion
        )
      }
    }
    const n = Object.keys(this._accessories).length
