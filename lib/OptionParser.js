...
class OptionParser extends events.EventEmitter {
  ...
  constructor (userInput = false) {
    ...
    this._debug('constructor(%j)', userInput)
  ...
  functionKey (key, f) {
    ...
    this._callbacks[key] = (value) => {
      this._object[key] = OptionParser.toFunction('f', f)
    }
  }
  ...
  hostKey (key = 'host', hostnameKey = 'hostname', portKey = 'port') {
    ...
    this._callbacks[key] = (value) => {
      const host = OptionParser.toHost(key, value, this._userInput)
  ...
  parse (options, defaults) {
    ...
    defaults = OptionParser.toObject('defaults', defaults)
    this._debug('parse(%j, %j)', options, defaults)
    ...
    this._debug('parse(%j, %j) ==> %j', options, defaults, this._object)
    return this._object
  }
}
