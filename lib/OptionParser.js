...
class OptionParser extends events.EventEmitter {
  ...
  static toHost (key, value, asString = false, userInput = false) {
    ...
    let hostname = list[0]
    if (hostname === '') {
      throw newRangeError(`${key}: missing hostname or IPv4 address`, userInput)
    }
    ...
    if (list.length < 2) {
      return asString ? hostname : { hostname: hostname }
    }
    let port = list[1]
    if (port === '') {
      throw newRangeError(`${key}: missing port`, userInput)
    }
    OptionParser.toString(`${key}: port`, port, true, userInput)
