...
module.exports = class OptionParser extends events.EventEmitter {
  ...
  static toBool (value, userInput = false) {
    ...
    if (value === 'false' || value === 'no' || value === 'off' || value === '0' || value === 0) {
      return false
    }
  ...
  static toPath (value, asString, userInput = false) {
    ...
    return asString ? prefix + list.join('/') : list
  }
    ...
    for (const key in options) {
      try {
        ...
        if (error instanceof UsageError) {
          this.emit('usageError', `${key}: ${error.message}`)
        } else {
  ...
  functionKey (key, f) {
    ...
    this._callbacks[key] = (value) => {
      this._object[key] = OptionParser.toFunction(value)
    }
  }
  ...
  // Define a key that takes a list of strings.
  listKey (key) {
    key = this._toKey(key)
    ...
    this._callbacks[key] = (value) => {
      const array = []
      const map = {}
      for (const element of OptionParser.toArray(value)) {
        try {
          OptionParser.toString(element, true, this._userInput)
          if (map[element]) {
            throw newRangeError(`duplicate element ${element}`, this._userInput)
          }
          map[element] = true
          array.push(element)
        } catch (error) {
  ...
  // Define a key that takes an object value.
  objectKey (key) {
    key = this._toKey(key)
    this._debug('objectKey(%j)', key)

    this._callbacks[key] = (value) => {
      this._object[key] = OptionParser.toObject(value, this._userInput)
    }
  }
