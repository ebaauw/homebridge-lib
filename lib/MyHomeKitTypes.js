...
class MyHomeKitTypes extends homebridgeLib.CustomHomeKitTypes {
  ...
  constructor (homebridge) {
    ...
    // Deprecated - use hap.Characteristic.LockPhysicalControls instead.
    this.createCharacteristic('Locked', uuid('035'), {
      format: this.formats.BOOL,
      perms: [this.perms.READ, this.perms.NOTIFY, this.perms.WRITE]
    }, 'Locked')

    // Used by homebridge-p1 in Electricity service.
    this.createCharacteristic('TotalConsumptionNormal', uuid('036'), {
      format: this.formats.FLOAT,
      unit: 'kWh',
      perms: [this.perms.READ, this.perms.NOTIFY]
    ...
    // Used by homebridge-p1 in Electricity service.
    this.createCharacteristic('TotalConsumptionLow', uuid('037'), {
      format: this.formats.FLOAT,
      unit: 'kWh',
      perms: [this.perms.READ, this.perms.NOTIFY]
    ...
    this.createCharacteristic('Streaming', uuid('038'), {
      format: this.formats.BOOL,
      perms: [this.perms.READ, this.perms.NOTIFY, this.perms.WRITE]
    })
    ...
    this.createCharacteristic('Time', uuid('04F'), {
      format: this.formats.STRING,
      perms: [this.perms.READ, this.perms.NOTIFY]
    }, 'Time')
    ...
    this.createService('Sonos', hap.Service.Switch.UUID, [
      ...
      this.Characteristic.SonosCoordinator
    ], [
      this.Characteristic.Tv
    ])
    ...
    this.createService('Speaker', hap.Service.Switch.UUID, [
      ...
      this.Characteristic.NightSound,
      this.Characteristic.SpeechEnhancement
    ])
  }
}
